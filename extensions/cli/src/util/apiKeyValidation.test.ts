...
describe("isValidAnthropicApiKey", () => {
  it("should return true for valid API keys", () => {
    ...
    expect(
      // Note: string intentionally avoids the GitHub secret-scanner pattern
      // `sk-ant-api03_` while still matching the key-format validation rules.
      isValidAnthropicApiKey("sk-ant-api03_xxxxxxxxxxxxxxxxxxxxxxxx"),
    ).toBe(true);
  });
